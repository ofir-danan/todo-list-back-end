...
const {readFileSync, writeFileSync, appendFileSync, unlinkSync, readdirSync, existsSync} = require("fs");
const app = express();
...
app.put("/b/:id", (req, res) => {
  ...
  try {
    if (!binExists) {
        throw new Error("bin does not exist with the given ID")
      }
    if (res.statusCode !== 200) {
      throw new Error(res.statusCode);
    }
...
app.get("/b/:id", (req, res) => {
  ...
  try {
    ...
    if (!binExists) { //a number, but doesn't exist
...
app.get("/b", (req, res) => {
    try {
      ...
      for (let i = 0; i < binsNamesList.length; i++) {
        ...
        try {
          const taskObject = readFileSync("./task/" + binName, {encoding: "utf8", flag: "r"});
          tasksList.push(JSON.parse(taskObject));
        } catch (e) {
          console.error(`Error: ${e} There is a problem with the GET ALL request`);          
        }
      }
      res.send(tasksList);
    } catch (e) {
...
app.delete("/b/:id", (req, res) => {
  try {
    if (response.statusCode !== 200) {
      ...
      unlinkSync("./task/" + req.params.id + ".json");
